...
public class GenUpdatePackageControl : UserControl
{
    ...
    private readonly string? _sourceRoot; // App_Data root for the workspace this tab was opened with
    ...
    public GenUpdatePackageControl(FileLookupService fileLookupService, Workspace? workspace)
    {
        ...
        if (workspace is not null && !string.IsNullOrWhiteSpace(workspace.SourcePath))
            _sourceRoot = Path.Combine(workspace.SourcePath, "App_Data");
    }
    ...
    /// rebuilds the Source File tree from that item's Link, the same resolution File Lookup
    /// itself uses (FileLookupService, searching every extension for the Link's basename).</summary>
    public void LoadForMenuItem(WCommandItem item)
    {
        ...
        {
            MessageBox.Show(this, $"Menu \"{item.Bar}\" không có Link gắn với source (có thể là mục nhóm/menu cha).",
                "Bcode — Gen Update");
            return;
        }

        var term = item.Link.TrimEnd('/', '\\').Split('/', '\\').Last();
        var root = _fileLookupService.BuildTree(_sourceRoot, extensionFilter: "", searchText: term, onlyShowFiltered: false);
    ...
    private void CreateUpdateFiles()
    {
        ...
        try
        {
            ...
            {
                // Preserve the file's App_Data-relative folder structure (Controllers/Filter/..., etc.)
                // inside the generated update folder, same layout FCode's own Gen Update produces.
                var relative = Path.GetRelativePath(_sourceRoot, sourceFile);
                var destFile = Path.Combine(destRoot, "App_Data", relative);
